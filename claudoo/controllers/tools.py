...
class AiAssistantTools(http.Controller):
    ...
    def _check_tool(self, tool):
        ...
        if tool not in request.ai_session._effective_tools():
    ...
    def _check_model(self, model):
        ...
        if model in request.env["claudoo.session"]._ai_excluded_models():
            raise AccessError(
    ...
    def _check_action_method(self, method):
        ...
        patterns = request.env["claudoo.session"]._ai_action_method_patterns()
    ...
    def _audit(self, started, vals):
        ...
        session = getattr(request, "ai_session", None)
        if session:
            vals["session_id"] = session.id
        vals["user_id"] = request.env.uid
        request.env["claudoo.tool_log"].sudo()._record(vals)
    ...
    # Tools
    # ------------------------------------------------------------------
    @http.route("/claudoo/tool/model_introspect", type="json",
                auth="ai_bridge", methods=["POST"])
    def model_introspect(self, model=None, **kw):
        ...
        def _do():
            ...
            eff = request.ai_session._effective_tools()
    ...
    @http.route("/claudoo/tool/orm_search_read", type="json",
                auth="ai_bridge", methods=["POST"])
    ...
    @http.route("/claudoo/tool/orm_read", type="json",
                auth="ai_bridge", methods=["POST"])
    ...
    @http.route("/claudoo/tool/orm_call", type="json",
                auth="ai_bridge", methods=["POST"])
    ...
    @http.route("/claudoo/tool/sql_select", type="json",
                auth="ai_bridge", methods=["POST"])
    def sql_select(self, query=None, params=None, max_rows=1000, **kw):
        ...
        def _do():
            ...
            # parameter 'claudoo.sql_statement_timeout_ms' (default 10s).
            try:
                timeout_ms = max(1000, int(request.env["ir.config_parameter"].sudo()
                                           .get_param("claudoo.sql_statement_timeout_ms", 10000)))
    ...
    # ------------------------------------------------------------------
    @http.route("/claudoo/tool/orm_create", type="json",
                auth="ai_bridge", methods=["POST"])
    ...
    @http.route("/claudoo/tool/orm_write", type="json",
                auth="ai_bridge", methods=["POST"])
    ...
    @http.route("/claudoo/tool/orm_unlink", type="json",
                auth="ai_bridge", methods=["POST"])
    ...
    # ------------------------------------------------------------------
    @http.route("/claudoo/tool/orm_action", type="json",
                auth="ai_bridge", methods=["POST"])
    ...
    @http.route("/claudoo/tool/run_wizard", type="json",
                auth="ai_bridge", methods=["POST"])
    ...
    @http.route("/claudoo/tool/run_server_action", type="json",
                auth="ai_bridge", methods=["POST"])
    def run_server_action(self, action=None, ids=None, **kw):
        ...
        def _do():
            ...
            allowed = request.env[
                "claudoo.session"]._ai_allowed_server_action_ids()
