...
class AiAssistantToolLog(models.Model):
    _name = "claudoo.tool_log"
    _description = "Claudoo AI Assistant Tool Call Audit Log"
    _order = "create_date desc, id desc"

    session_id = fields.Many2one(
        "claudoo.session", ondelete="set null", index=True)
    ...
    def _record(self, vals):
        ...
        try:
            with odoo.registry(dbname).cursor() as cr:
                ...
                env["claudoo.tool_log"].create(vals)
                cr.commit()
        except Exception:
            _logger.exception("Failed to write claudoo.tool_log")
