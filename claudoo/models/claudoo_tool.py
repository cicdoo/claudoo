...
class AiAssistantTool(models.Model):
    ...
    One record per tool (seeded from data/claudoo_tool_data.xml and kept in
    sync with the READ_TOOLS/WRITE_TOOLS code constants in claudoo_session).
    It is the join target for the per-user `res.users.claudoo_tool_ids`
    Many2many that decides which tools each user may invoke.
    """
    _name = "claudoo.tool"
    _description = "Claudoo AI Assistant Tool"
