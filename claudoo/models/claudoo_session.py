...
# Hard floor: every Claude Code built-in tool is denied so the model can ONLY act
# through our mcp__odoo__* tools. --allowedTools merely auto-approves; it does not
# disable un-listed tools (read-only ones like ToolSearch/Glob run without a prompt),
# so we must enumerate the full built-in surface here. Crucially this includes the
# agent/skill spawners (Task*, Skill, Workflow) which could otherwise reach Bash.
# NB: only list tool names the running CLI actually knows — an unknown name makes
# the CLI print "Permission deny rule '<x>' matches no known tool" to stderr,
# which the runner then captures into last_error and obscures real failures.
# `MultiEdit` (folded into `Edit`) and `SlashCommand` were dropped in newer CLI
# builds (>= 2.1.x), so they are intentionally NOT listed here.
DISALLOWED_BUILTINS = [
    "Bash", "Write", "Edit", "NotebookEdit", "Read",
    "Glob", "Grep", "WebFetch", "WebSearch",
    "Task", "TaskCreate", "TaskGet", "TaskList", "TaskOutput", "TaskStop",
    "TaskUpdate", "Skill", "Workflow", "ToolSearch", "AskUserQuestion",
    "EnterPlanMode", "ExitPlanMode", "EnterWorktree", "ExitWorktree",
    "Monitor", "PushNotification", "ScheduleWakeup", "RemoteTrigger",
    "CronCreate", "CronDelete", "CronList", "BashOutput", "KillShell",
    "TodoWrite", "ListMcpResources", "ReadMcpResource",
]

DEFAULT_CLI_GLOB = (
    "/opt/odoo/.local/share/code-server/extensions/"
...
class AiAssistantSession(models.Model):
    _name = "claudoo.session"
    _description = "Claudoo AI Assistant Conversation Session"
    ...
    claude_session_id = fields.Char(
        string="Claude Session UUID", copy=False, index=True,
        help="UUID passed to the Claude CLI (--session-id / --resume) for continuity.",
    )
    ...
    message_ids = fields.One2many(
        "claudoo.message", "session_id", string="Messages")
    ...
    def _get_scratch_dir(self):
        self.ensure_one()
        root = self._config("scratch_root", "/var/lib/odoo/claudoo_scratch")
        path = os.path.join(root, self.claude_session_id)
        try:
    ...
    def _effective_tools(self):
        ...
        self.ensure_one()
        return self.user_id.sudo()._ai_effective_tools()
    ...
    def _claude_session_exists(self, config_dir):
        """True if this conversation's history exists in ``config_dir``.

        The CLI can only ``--resume`` a session whose transcript lives under the
        current CLAUDE_CONFIG_DIR. Sessions started against a different config
        dir (e.g. the old shared HOME, before per-user auth) won't be found, so
        we must fall back to starting a fresh CLI session instead of resuming.
        """
        self.ensure_one()
    ...
    def _resolve_cli_path(self):
        """Explicit config param, else newest match of the glob, else error."""
        ...
        if not matches:
            raise UserError(_(
                ...
                "'claudoo.cli_path' or 'claudoo.cli_glob'. Tried: %s"
            ) % pattern)
        ...
        return sorted(matches, key=_ver_key)[-1]

    def _oauth_env(self):
        """Return env vars pointing the CLI at THIS user's private .claude dir.

        Each user authenticates with their own Claude subscription (see
        res.users._ai_oauth_*), and their OAuth credentials live in
        `<home_root>/<uid>/.claude/.credentials.json`. Isolating HOME and
        CLAUDE_CONFIG_DIR per user keeps credentials, session state and settings
        fully separate — and lets the CLI refresh that user's token in place.
        """
        self.ensure_one()
        user = self.user_id.sudo()
        config_dir = user._ai_config_dir(create=True)
        return {
            "HOME": user._ai_home_dir(),
    ...
    def _verify_bridge_token(self, token):
        ...
        try:
            ...
            return None
        if payload.get("scope") != "ai_bridge":
            return None
    ...
    def send_message(self, body, attachment_ids=None):
        ...
        # a separate `claude` subprocess (~hundreds of MB), so too many at once can
        ...
        # _gc_stale cron, so a crashed run cannot wedge a slot permanently.
        ...
        if not self.user_id.sudo()._ai_is_authenticated():
            raise UserError(_(
                "You need to connect your Claude account before chatting. "
                "Click “Login with Claude” to authenticate."))

        paths, atts = self._materialize_attachments(attachment_ids)
        if not body and not atts:
            raise UserError(_("Empty message."))

        user_msg = self.env["claudoo.message"].create({
            "session_id": self.id,
            "role": "user",
            "body": body,
    ...
    def _build_prompt(self, body, paths):
        ...
        note = _(
            "The user attached the following file(s), saved in your working "
            "directory. Use the Read tool to open them — text/CSV/code is read "
            "verbatim, images are viewed visually, and PDFs page-by-page:\n%s"
        ) % listing
