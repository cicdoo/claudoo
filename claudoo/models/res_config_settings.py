...
class ResConfigSettings(models.TransientModel):
    ...
    claudoo_sql_enabled = fields.Boolean(
    ...
    claudoo_zero_trust_default = fields.Boolean(
    ...
    claudoo_action_method_patterns = fields.Char(
    ...
    claudoo_excluded_model_ids = fields.Many2many(
    ...
    claudoo_allowed_server_action_ids = fields.Many2many(
    ...
    def get_values(self):
        ...
        res["claudoo_excluded_model_ids"] = [(6, 0, models.ids)]
        sa_ids = self.env["claudoo.session"]._ai_allowed_server_action_ids()
        res["claudoo_allowed_server_action_ids"] = [(6, 0, list(sa_ids))]
        return res

    def set_values(self):
        super().set_values()
        names = ",".join(sorted(self.claudoo_excluded_model_ids.mapped("model")))
        ICP = self.env["ir.config_parameter"].sudo()
        ICP.set_param("claudoo.excluded_models", names)
        ICP.set_param("claudoo.server_action_ids",
                      ",".join(map(str, sorted(self.claudoo_allowed_server_action_ids.ids))))
