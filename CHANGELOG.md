...
Odoo-style versioning. Add new entries under `## [Unreleased]`; a maintainer assigns
...
- **Rendered charts & HTML reports in chat** — the assistant can now produce
  ...
  `report` message role plus a `claudoo.session._post_report(html)` helper let
...
- **Namespaced all `res.config.settings` and `res.users` fields** from the
  ...
  A pre-migration (`17.0.1.0.1`) preserves each user's zero-trust selection
...
[17.0.1.0.1]: https://github.com/cicdoo/claudoo/releases/tag/17.0.1.0.1
